...
import type { LayerListItem } from '#/shared/types/geonodeTypes';
...
<template>
	<div class="tw:p-2 tw:rounded-md">
			...
			>
				<div class="tw:flex tw:gap-2 tw:py-3 tw:border-b tw:border-slate-300 ">
					<div class="tw:w-[136px] tw:min-w-[136px] tw:flex tw:justify-center">
						...
						>
							<template #placeholder>
								<div class="tw:flex tw:items-center tw:justify-center tw:w-full tw:h-full tw:bg-slate-200">
									<span class="tw:text-slate-500 tw:font-semibold tw:text-lg">no image</span>
								</div>
							</template>
						</v-img>
					</div>
					...
					<div class="tw:min-w-0 tw:flex-1">
						<p class="tw:text-[1.05rem] tw:font-semibold tw:text-[#111111] tw:leading-tight tw:mb-1">
							{{ layer.title }}
						</p>
						...
						tw:gap-x-3 tw:gap-y-1 dark-rose-text tw:text-[0.82rem]">
							<span class="tw:inline-flex tw:items-center tw:gap-1">
								<v-icon size="14">mdi-account</v-icon>{{ layer.owner_username }}
							</span>
							<span class="tw:inline-flex tw:items-center tw:gap-1">
								<v-icon size="14">mdi-calendar-month-outline</v-icon>{{ formatDate(layer.created) }}
							</span>
							<span class="tw:inline-flex tw:items-center tw:gap-1">
								<v-icon size="14">mdi-eye-outline</v-icon>{{ layer.popular_count }}
							</span>
							<span class="tw:inline-flex tw:items-center tw:gap-1">
		...
		<p v-else-if="!props.loading" class="tw:text-sm tw:text-gray-500">Nessun layer disponibile.</p>
	</div>
</template>
